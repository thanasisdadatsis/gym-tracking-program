...
const logGymSession = (session, user) => {
...
// --- Metrics Evaluation ---
const returnStepsRating = (session) => {
    if (session.steps >= 20_000)
        return "excellent";
    if (session.steps >= 10_000)
        return "very Good";
    if (session.steps >= 5_000)
        return "mid";
    return "bad";
};
const returnKcalRating = (session) => {
    if (session.kcal >= 1000)
        return "excellent";
    if (session.kcal >= 500)
        return "good";
    if (session.kcal >= 150)
...
const evaluateExercisesStatus = (session) => {
...
const generateCompleteSummary = (user, session, includeSteps, includeKcal, includeExercises) => {
